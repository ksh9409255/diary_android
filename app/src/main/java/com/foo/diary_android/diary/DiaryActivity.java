...
public class DiaryActivity extends AppCompatActivity {
    ...
    private View.OnClickListener writeListener = new View.OnClickListener() {
        public void onClick(View v) {
            Intent intent = new Intent(DiaryActivity.this, LookActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        service.getFriend(getIntent().getLongExtra("friendId",0L),MainActivity.accessToken,user.getId()).enqueue(new Callback<MemberDto>() {
            @Override
            public void onResponse(Call<MemberDto> call, Response<MemberDto> response) {
                nickName.setText(response.body().getNickName());
                characterImg.setImageResource(MainActivity.characterStore.get(response.body().getCategoryId()));
                emoticonBox.setOnClickListener(l->{
                    Intent intent = new Intent(getApplicationContext(), EmoticonActivity.class);
                    intent.putExtra("user",response.body());
                    startActivity(intent);
                    overridePendingTransition(R.anim.anim_slide_in_right, R.anim.anim_slide_in_left);
                });
                service.findAllDiary(response.body().getId(),MainActivity.accessToken).enqueue(new Callback<List<DiaryFindDto>>() {
        ...
        characterImg = findViewById(R.id.diary_character_img);
        ...
        materialCalendarView.setOnDateChangedListener(new OnDateSelectedListener() { // 날짜를 클릭시 발생하는 이벤트 처리
            @Override // 구현해야 하는 메소드
            public void onDateSelected(@NonNull MaterialCalendarView widget, @NonNull CalendarDay date, boolean selected) {
                ...
                if(calendarDays.containsKey(click_date)){
                    ...
                    dialog = new CalendarDialog(DiaryActivity.this,click_date
                            ,userCalendarData,writeListener,okListener);
                    dialog.show();
                }
            }
        });
